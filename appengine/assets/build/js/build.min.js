...
$(document).ready(function(){"use strict";var e=($("#select-platform"),$("#select-activity"),$("#select-microphone"),$("#select-lookingfor"),$("#select-level"),$("#select-dzlevel"),$("#select-search"),$("#aa-username")),a=$("#aa-platform"),n=$("#aa-activity"),o=$("#aa-microphone"),t=$("#aa-lookingfor"),l=$("#aa-level"),i=$("#aa-dzlevel"),r=$("#aa-description"),s=$("#save-agent"),c=$("#search-agents"),d='<i class="fa fa-plus-circle"></i> Agregar agente',u='<i class="fa fa-spinner fa-spin"></i> Agregando, espera...',v='<i class="fa fa-search"></i> Buscar Agentes',p='<i class="fa fa-spinner fa-spin"></i> Buscando, espera...',g=$("#add-alert-placement"),m=$("#modal-add-agent"),f=$("<div>").attr({"class":"alert alert-danger",role:"alert",style:"display:none"}).append($("<button>").attr({type:"button","class":"close","data-dismiss":"alert","aria-label":"close"}).append($("<span>").attr({"aria-hidden":"true"}).html("&times;"))),h="<strong>Oh no!</strong> El <u>nombre de usuario</u> es incorrecto o es demasiado corto. Verifícalo y vuelve a intentarlo.",y="<strong>Oh no!</strong> No has seleccionado <u>una plataforma</u>. Elige una y vuelve a intentarlo.",b="<strong>Oh no!</strong> No has seleccionado <u>una actividad</u>. Elige una y vuelve a intentarlo.",x="<strong>Oh no!</strong> No has indicado si <u>tienes o no un micrófono</u>. Elige una opción y vuelve a intentarlo.",O="<strong>Oh no!</strong> No has indicado <u>qué buscas en otros agentes</u>. Elige una opción y vuelve a intentarlo.",E="<strong>Oh no!</strong> No has indicado <u>qué nivel tienes en el modo historia</u>. Elige una opción y vuelve a intentarlo.",q="<strong>Oh no!</strong> No has indicado <u>qué nivel tienes en la Zona Oscura</u>. Elige una opción y vuelve a intentarlo.",N="<strong>Oh no!</strong> No has entregado <u>información adicional</u> o escribiste poca información, como tu DPS o tu nivel de Salud. Escribe un detalle y vuelve a intentarlo.",I=function(e,a){m.scrollTop(0),e.empty().append(f.clone().append(a).fadeIn("fast"))},k=function(e,a){e.is(":disabled")?(e.prop("disabled",!1),e.html(e.attr("data-original")),e.attr("data-original","")):(e.prop("disabled",!0),e.attr("data-original",e.html()),e.html(a))};s.click(function(c){var v=function(e){I(g,e),k(s,d)};if(k(s,u),""===$.trim(e.val())||$.trim(e.val()).length<6)return void v(h);if(null==a.val())return void v(y);if(null==n.val())return void v(b);if(null==o.val())return void v(x);if(null==t.val())return void v(O);if(null==l.val())return void v(E);if(null==i.val())return void v(q);if(""===$.trim(r.val())||$.trim(r.val()).length<6)return void v(N);var p=$.post("/addnew",{username:$.trim(e.val()),platform:a.val(),activity:n.val(),microphone:o.val(),lookingfor:t.val(),level:l.val(),dzlevel:i.val(),description:r.val()});p.done(function(c){console.log(c),e.val(""),a.prop("selectedIndex",0),n.prop("selectedIndex",0),o.prop("selectedIndex",0),t.prop("selectedIndex",0),l.prop("selectedIndex",0),i.prop("selectedIndex",0),r.val(""),g.empty(),k(s,d),m.modal("toggle");var u=window.location.protocol+"//"+window.location.host+c.deletionid,v="<p><strong>Ya estás agregado, "+c.username+'!</strong> Cuando otros agentes busquen por las características que mencionaste, tu nombre aparecerá en el listado. <strong style="color:red">Si quieres eliminar tu nombre del listado</strong>, puedes conservar este link en un lugar seguro. Una vez que quieras eliminarte, ábrelo y sigue las instrucciones:<p><div class="form-group"><input readonly type="text" class="form-control" value="'+u+'"></div>';bootbox.dialog({title:"Agente agregado! Bienvenido al Listado",message:v,buttons:{success:{label:"Entendido!",className:"btn-primary"}}})}),p.fail(function(e){""!=e.responseText&&v("<strong>Oh no!</strong> "+e.responseText)})}),c.click(function(e){k(c,p),setTimeout(function(){k(c,v)},5e3)})});